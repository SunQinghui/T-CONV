...
class TaxiDataset(H5PYDataset):
    def __init__(self, which_set, filename='mydata.hdf5', **kwargs):
...
class TaxiStream(DataStream):
    def __init__(self, which_set, filename='mydata.hdf5', iteration_scheme=None, **kwargs):
...
def origin_call_unnormalize(x):
    if _origin_calls is None:
        _origin_calls = h5py.File(os.path.join(data.path, 'mydata.hdf5'), 'r')['unique_origin_call']
...
def taxi_id_unnormalize(x):
    if _taxi_ids is None:
        _taxi_ids = h5py.File(os.path.join(data.path, 'mydata.hdf5'), 'r')['unique_taxi_id']
    return _taxi_ids[x]
...
def taxi_it(which_set, filename='mydata.hdf5', sub=None, as_dict=True):

...
class Stream (object):
	...
	def __init__(self, trip_id, call_type, origin_call, origin_stand, taxi_id, \
		...
		self.min_latitude = data.train_gps_mean[0] - data.train_gps_std[0]
		self.max_latitude = data.train_gps_mean[0] + data.train_gps_std[0]
		self.min_longitude = data.train_gps_mean[1] - data.train_gps_std[1]
		self.max_longitude = data.train_gps_mean[1] + data.train_gps_std[1]

		self.dev_latitude = (self.max_latitude - self.min_latitude)/dim
		self.dev_longitude = (self.max_longitude - self.min_longitude)/dim
	def get_size(self):
		return self.size
	...
	def at_least_k(self,k, v, pad_at_begin, is_longitude):
	    if len(v) == 0:
	    ...
	    self.griding(k,v,is_longitude)
	    return v

	def griding(self, k, v, is_longitude):
 		if is_longitude == False:
 			#print (v,self.dev_latitude,self.min_latitude, self.max_latitude)
 			v //= self.dev_latitude
 			v *=  self.dev_latitude
 			#print (v)
 			#exit()
 		else:
 			#print (v,self.dev_longitude,self.min_longitude, self.max_longitude)
 			v //= self.dev_longitude 
			v *=  self.dev_longitude
			#print (v)
			#exit()
	...
	def get_all_data(self):
		...
		for i in range(self.size):
			...
			first_k_latitude_[i]= self.normalize(self.at_least_k(self.k, self.latitude[i], False, False)[:self.k], True)
			first_k_longitude_[i]= self.normalize(self.at_least_k(self.k, self.longitude[i], False, True)[:self.k], False)
			last_k_latitude_[i] = self.normalize(self.at_least_k (self.k, self.latitude[i], True, False)[-self.k:], True)
			last_k_longitude_[i] = self.normalize(self.at_least_k(self.k, self.longitude[i], True, True)[-self.k:], False)
		...
		for j in range(lsize):
			...
			for k in range(batchsize):
				if i < self.size:				
					...
					first_k_latitude_[k]= self.at_least_k(self.k, self.latitude[i], False, False)[:self.k]
					first_k_longitude_[k]= self.at_least_k(self.k, self.longitude[i], False, True)[:self.k]
					last_k_latitude_[k] = self.at_least_k (self.k, self.latitude[i], True, False)[-self.k:]
					last_k_longitude_[k] = self.at_least_k(self.k, self.longitude[i], True, True)[-self.k:]
					...
					first_k_latitude_[k]= self.at_least_k(self.k, self.latitude[0], False, False)[:self.k]
					first_k_longitude_[k]= self.at_least_k(self.k, self.longitude[0], False, True)[:self.k]
					last_k_latitude_[k] = self.at_least_k (self.k, self.latitude[0], True, False)[-self.k:]
					last_k_longitude_[k] = self.at_least_k(self.k, self.longitude[0], True, True)[-self.k:]
					snapshot_[k][0] = self.get_snapshot(self.k, first_k_latitude_[k], first_k_longitude_[k], \
									last_k_latitude_[k], last_k_longitude_[k], self.conv_dim)
	...
	def get_sample_data(self, k, bnormalize = True): # get a row of the data, and the sub-traj with randomized length is selected
		...
		if bnormalize == True:
			first_k_latitude_= self.normalize(self.at_least_k(k, self.latitude[self.id][:n], False, False)[:k], True)
			first_k_longitude_= self.normalize(self.at_least_k(k, self.longitude[self.id][:n], False, True)[:k], False)
			last_k_latitude_ = self.normalize(self.at_least_k (k, self.latitude[self.id][:n], True, False)[-k:], True)
			last_k_longitude_ = self.normalize(self.at_least_k(k, self.longitude[self.id][:n], True, True)[-k:], False)
		else:
			first_k_latitude_= self.at_least_k(k, self.latitude[self.id][:n], False, False)[:k]
			first_k_longitude_= self.at_least_k(k, self.longitude[self.id][:n], False, True)[:k]
			last_k_latitude_ = self.at_least_k (k, self.latitude[self.id][:n], True, False)[-k:]
			last_k_longitude_ = self.at_least_k(k, self.longitude[self.id][:n], True, True)[-k:]

...
class CNN (object):
    ...
    def __init__(self, input, image_shape):
        ...
        layer0 = LeNetConvPoolLayer(
            ...
            filter_shape=(nfilters[0], 1, filter_size[0], filter_size[1]),
            poolsize= poolsize_
        )
...
class Embedding_Conv_Mlp(object):
    def __init__(self, image_shape,snapshot, origin_call, origin_stand, week_of_year, \
        ...
        self.mlp = MLP(
            rng=rng,
            input= mlp_input,
            n_in=self.cnn.outputdim + 7*10, #embedding + trajectory
            n_hidden= 500,
            n_out=2
        )
...
def test_conv_mlp(learning_rate=0.0005, n_epochs=10, batch_size=200):
    conv_dim = 30
    
    image_shape = [batch_size,1,conv_dim,conv_dim]

    #dataset
    dataset = load_dataset.Dataset('../../../data/','../../../logs/log_load_file_g')

    #load twice from 400000 ->
    '''
    for i in range((374/15) % 4):
    ...
    save_loader = SaveLoadParams('conn_global_real')
    base_iter=1
    '''
    base_iter = 3749999#  2409999 #149999
    ...
    epoch = 0
    done_looping = False
    #test = runOnTest('conn_global_real')
    real_epoch = 0

    log_file = open('conn_global_real_log', 'w')
    ...
    while epoch < n_epochs:
        ...
        for minibatch_index in range(n_train_batches):
            #print("print %s minibatch_index" %minibatch_index)
            [real_epoch,origin_call_train_, origin_stand_train_, day_type_train_,taxi_id_train_, week_of_year_train_, \
                day_of_week_train_,qhour_of_day_train_, snapshot_train_, dest_train_] \
            ...
            if (iter) % validation_frequency == 0:
                ...
                for i in range(n_valid_batches):
                    [valid_epoch, origin_call_valid_, origin_stand_valid_, day_type_valid_,taxi_id_valid_, \
                        ...
                        snapshot_valid_, dest_valid_] = valid_stream.get_data_batch_conv(batch_size)
